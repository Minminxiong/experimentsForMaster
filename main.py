...
def main(args):
    ...
    features_name = ["target_node", "target_neighbors_num", "First_nbr_adopter", "Second_nbr_adopter", \
                     ...
                     ["cas_cos_simi_{}".format(i) for i in range(args.k)] + ["adoption_p", "label"]
...
#                tf = features.Target_Features(0, rwr)
                ...
                for tf_list in tf_infected:
                    feature_list1 = tf_list[: -3 * args.k -1] + feature_list + tf_list[-3* args.k -1:] + [1]
                    feature_writer.writerow(feature_list1)
                    
                for tf_list in tf:
                    feature_list1 = tf_list[: -3 * args.k -1] + feature_list + tf_list[-3* args.k -1:] + [0]
                    feature_writer.writerow(feature_list1)

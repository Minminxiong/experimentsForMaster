...
class Features():
    ...
    start = time.time()
    cas_timeline_embeddings = read_embeddings_dict("../embeddings/cas_timeline.embeddings")
...
#        temporal_features.extend(time_1_i)
        ...
        if self.cas_timeline_embeddings.__contains__(target_node):
            ...
            for adoption_node in self.unique_Ak:
                if self.cas_timeline_embeddings.__contains__(adoption_node):
                    cas_consine_similarity_firstKV_target.append(self.cosine_similarity(self.cas_timeline_embeddings[adoption_node], target_timeline_embedding))
                else:
            ...
            cas_consine_similarity_firstKV_target = [random.uniform(0, 0.1) for i in range(len(self.unique_Ak))]
        # =============================================================================
        #         for adoption_node in self.unique_Ak:
    ...
    #                             "com_hashtags_ave",]
        ...
        distKV_target, cas_consine_similarity_firstKV_target = self.get_reachability_features(target_node)
